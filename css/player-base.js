...
customVideoJsUI.innerHTML = `.vjs-loading-spinner {
  width: 64px !important;
  height: 64px !important;
  border: 3px solid rgba(255, 255, 255, 0.25);
  border-top-color: #fff;
  border-radius: 50%;
  animation: vjs-buffer-spin 0.8s linear infinite;
  position: absolute;
  top: 50%;
  left: 50%;
...
@keyframes vjs-buffer-spin {
  from { transform: rotate(0deg); }
  to { transform: rotate(360deg); }
}

.vjs-loading-spinner.vjs-hidden {
  opacity: 0 !important;
  transition: opacity 0.25s ease;
}

.vjs-loading-spinner:not(.vjs-hidden) {
  opacity: 1 !important;
  transition: opacity 0.25s ease;
}
